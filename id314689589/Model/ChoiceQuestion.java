...
public class ChoiceQuestion extends Questions implements Serializable {
	...
	public boolean addAnswer(Answers newAnswer) {
		for (int i = 0; i < allAnswers.size(); i++)
			if (allAnswers.get(i).getAnswer().equalsIgnoreCase(newAnswer.getAnswer()))
				return false;
	...
	public boolean addDefultAnswers() {
		this.addAnswer(new Answers("more then one answer is correct", this.numberOfCorrectAnswers() > 1));
		this.addAnswer(new Answers("no answer is correct", (this.numberOfCorrectAnswers() == 0)));
		return true;
	}
